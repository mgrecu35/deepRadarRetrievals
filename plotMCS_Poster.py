...
fhC=Dataset('../XPlainable/data/2B.GPM.DPRGMI.CORRA2018.20180625-S041042-E054316.024557.V06A.HDF5')
fh=Dataset(fname,'r')
...
fhKu=Dataset('../XPlainable/data/2A.GPM.Ku.V8-20180723.20180625-S041042-E054316.024557.V06A.HDF5')
...
for i in range(nx):
    for j in range(12,37):
        ...
        if pType[i,j]==2:
            ...
            zKucL.append(zKuC[i,j,nzb1-60:nzb1+26])
            ind1=np.argmax(zKum[i,j,nzb1-20:nzb1+16])
            if zKum[i,j,nzb1-20:nzb1+10].max()>42:
                maxHL.append([zKum[i,j,nzb1-20+ind1],36-ind1])
                if zKum[i,j,nzb1-20+ind1]>45:
                    print(zKum[i,j,nzb1-20:nzb1+10],36-ind1)
                #stop
...
plt.figure()
plt.hist2d(np.array(maxHL)[:,0],np.array(maxHL)[:,1],bins=(35+np.arange(20),range(30)),norm=matplotlib.colors.LogNorm(),cmap='jet')
...
for i in range(n1):
    a=np.nonzero(winL==i)
    zKuClass[i]=zKuL[a[0],:].mean(axis=0)
    plt.plot(zKuClass[i,::-1],(-26+np.arange(nz))*0.125)

plt.savefig('mcs_zKu_class.png')
plt.figure()
plt.plot(zKuL[:,::-1].mean(axis=0),(-26+np.arange(nz))*0.125)
plt.grid()
plt.savefig('meanZ_MCS.png')
stop
